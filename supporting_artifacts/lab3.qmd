...
This data set consists of data from a study on African-American English (AAE), in which the researcher wanted to explore how musical tastes and familiarity with AAE were related. Many data variables were collected on the participants of the study, such as their sex, ethnicity, and hometown population, as well as their preferred musical genres and familiarity with pop culture knowledge. The dependent variable is familiarity, which is a score from 1-5 on how familiar the participant was with the AAE term in question.
...
Each row is a AAE vocabulary term asked to each of the participants.
...
Some missing values were replaced with 0. Benefits of this are that there will be less errors when performing computations on the data. Drawbacks are that it may lead to less accurate results when computations are performed; for example, performing mean on population data with missing variables replaced by 0s will skew the mean to be less than it should be.

Missing values in the \*Move and blackWeekly variables were replaced with mean values. Benefits of this are that it would allow these missing values to be included in computations and not skew the mean. Drawbacks of this are that it assumes that the missing variables would have an average value which may or may not be accurate.
...
Representing the data in this way can have issues because this generalizes certain ethnic or racial groups into one group, even thought they may not necessarily have similar traits in relation to the data. It's possible that data only pertaining to one ethnic or racial group may be lost because they are only looked at as part of the larger group.
...
The people in this study consist of 117 females and 51 males. The average age is 20, and the median age is 19. The people in this study were almost all white, with 135 white participants, and those who were not white were either African, Asian, biracial, Black, Hispanic, Indian, Native American, or unknown.
...
```{r}
# code chunk for Q10
ggplot(data = hiphop_demos, mapping = aes(x = age)) +
  geom_histogram(binwidth = 1, fill = "orange") +
  labs(x = "Age of participant", y = "Count", title = "Distribution of participant age")
ggplot(data = hiphop_demos, mapping = aes(x = age, y = sex)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(color = "darkblue") +
  labs(x = "Age of participant", y = "Sex of participant", 
       title = "Distribution of age to sex of participants")
```
...
"Off the hook" was the most familiar, while "catch the vapors" was the least familiar.
...
word_familiarity |>
  slice_max(avg_familiarity)
word_familiarity |>
  slice_min(avg_familiarity)
```

**12. Non-white women.**

"Feel me" was the most familiar, and "break someone out", "dukey rope", "plex", and "rolie" were the least familiar.
...
"5-0" was the most familiar, and "ay yo trip", "beezy", and "break someone out", among many others, were the least familiar.

```{r}
# code chunk for Q13
word_familiarity <- hiphop_clean |>
  filter(eth == "white" & sex == "Male" & age > 30) |>
  group_by(word) |>
  summarize(avg_familiarity = mean(as.numeric(familiarity))) 
word_familiarity |>
  slice_max(avg_familiarity)
...
Participant 17 might be secretly Bieber. I filtered by white males age 17-23 in a city of population 10,000-60,000 who can name 5 Justin Bieber songs, and got p17.
